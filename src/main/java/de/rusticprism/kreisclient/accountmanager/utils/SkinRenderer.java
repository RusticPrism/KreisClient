...
public class SkinRenderer {
			...
			try {
				...
				if (uuid == null) {
					try (InputStreamReader isr = new InputStreamReader(new URL("https://api.mojang.com/users/profiles/minecraft/" + name).openStream(), StandardCharsets.UTF_8)) {
						uuid = UUIDTypeAdapter.fromString(KreisClient.GSON.fromJson(isr, JsonObject.class).get("id").getAsString());
					}
				}
				String base64Data;
				try (InputStreamReader isr = new InputStreamReader(new URL("https://sessionserver.mojang.com/session/minecraft/profile/" + UUIDTypeAdapter.fromUUID(uuid)).openStream(), StandardCharsets.UTF_8)) {
					base64Data = KreisClient.GSON.fromJson(isr, JsonObject.class).getAsJsonArray("properties").get(0).getAsJsonObject().get("value").getAsString();
				}
				JsonObject jo = KreisClient.GSON.fromJson(new String(Base64.getDecoder().decode(base64Data), StandardCharsets.UTF_8), JsonObject.class).getAsJsonObject("textures").getAsJsonObject("SKIN");
				...
				try {
					...
					th.addSuppressed(t);
					KreisClient.LOGGER.warn("Unable to load skin: " + name, th);
					return;
				}
			}
			try {
				...
				KreisClient.LOGGER.warn("Unable to make model of skin: " + name, t);
			}
		}
		if (!faceF.exists()) {
			...
			try {
				...
				KreisClient.LOGGER.warn("Unable to make face from skin model: " + name, t);
			}
		}
	}
	...
	public static class SkinsLoadingOverlay extends Overlay {
		private static final Identifier PROGRESS = new Identifier("kreisclient", "backgrounds/splash.png");
